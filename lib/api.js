...
// @param [options.api_key] {String} Your App's API Key (Public and Private Apps).  This can be provided on the server (instead of a keyset), and on the client if `options.insecure` is `true`.
...
var API = Shopify.API = function ShopifyAPI(options) {
    ...
    if (Meteor.isClient) {
        if (!options.keyset && !options.insecure) {
            throw new Error("Missing required `options.keyset`.  Use `options.insecure=true` to disable this error.");
        }

        if (options.api_key && !options.insecure) {
            console.warn("Don't expose your App's api_key on the client!  Use `options.keyset` instead and `Shopify.addKeyset` on the server.  Use `options.insecure=true` to disable this warning.");
        }
...
if (Meteor.isServer) {
    Meteor.methods({
        ...
        "__ShopifyCall": function(name, options, config) {
...
API.define = function(info) {
    ...
    if (Meteor.isServer) {
        ...
        API.prototype[info.name] = function(options, callback) {
            ...
            Meteor.call("__ShopifyCall", info.name, options, this.config, callback);
        };
    }
};
